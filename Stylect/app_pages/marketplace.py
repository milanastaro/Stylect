...
from utils.shop_engine import (
    search_products, get_user_shop_items, save_user_shop_items, recommend_bundles, generate_ai_listing
)
from utils.trade_engine import (
    get_pending_trades, respond_to_trade, get_trade_history,
    get_trade_reputation, generate_dual_shipping_labels
)
import uuid, os
...
def show():
    ...
    with tab3:
        ...
        for tid, t in history.items():
            ...
            if t["status"] == "accepted":
                ...
                if st.button("📝 Submit Review", key=f"submit_{tid}"):
                    from utils.trade_engine import submit_trade_review
        ...
        if selected_user:
            from utils.trade_engine import get_trade_reviews, calculate_trade_rating
...
# ---------------------- 👗 STYLING SERVICES ---------------------- #
with tab4:
    from utils.styling_services import (
